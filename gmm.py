...
def segment_with_gmm(image_path, n_components=3):
    # 1. Load Image
    ...
    # 3. Build Gaussian Mixture Models
    pixels = img_rgb.reshape(-1, 3).astype(np.float64)
    fg_pixels = pixels[fg_mask.flatten()]
    bg_pixels = pixels[bg_mask.flatten()]

    gmm_fg = GaussianMixture(n_components=n_components, covariance_type='full', reg_covar=1e-4)
    gmm_bg = GaussianMixture(n_components=n_components, covariance_type='full', reg_covar=1e-4)

    gmm_fg.fit(fg_pixels)
    gmm_bg.fit(bg_pixels)

    # 4. Unary Costs (Data Term)
    # score_samples returns log-likelihood, multiply by -1 for costs
    unary_fg = -gmm_fg.score_samples(pixels).reshape(h, w).astype(np.float32)
    unary_bg = -gmm_bg.score_samples(pixels).reshape(h, w).astype(np.float32)
    ...
    plt.subplot(1, 2, 1), plt.title("GMM Unary Costs (BG Likelihood)"), plt.imshow(unary_bg, cmap='hot')
    plt.subplot(1, 2, 2), plt.title("Extracted Object"), plt.imshow(extracted)
    plt.show()

# Run the function
if __name__ == "__main__":
    segment_with_gmm("/home/radhika/Documents/CV-assignment-2/images/object.jpeg") # Replace with your image